...
MTK::get_cov<process_noise_ikfom>::type process_noise_cov() {
  ...
  MTK::setDiagonal<process_noise_ikfom, vect3, 6>(
      cov, &process_noise_ikfom::nbg, 0.00001);  // *dt 0.00001 0.00001 * dt *dt 0.3 //0.001 0.0001 0.01
...
///@brief 计算输入(加速度计和陀螺仪去零偏)
Eigen::Matrix<double, 24, 1> get_f(state_ikfom& s, const input_ikfom& in) {
  Eigen::Matrix<double, 24, 1> res = Eigen::Matrix<double, 24, 1>::Zero();
  vect3 omega;
  in.gyro.boxminus(omega, s.bg);               /// 陀螺仪去零偏
  vect3 a_inertial = s.rot * (in.acc - s.ba);  /// 加速度计去除零偏转为世界坐标系
  for (int i = 0; i < 3; i++) {
    res(i) = s.vel[i];
    res(i + 3) = omega[i];
